 ...
 *
 * Cikon IoT Ethernet - W5500 SPI Backend
...
static esp_err_t w5500_init_spi_bus(void) {
    ...
    ESP_LOGI(TAG, "Initializing SPI bus (host=%d, SCLK=%d, MOSI=%d, MISO=%d)",
             CONFIG_W5500_SPI_HOST, 
             CONFIG_W5500_SPI_SCLK_GPIO,
             CONFIG_W5500_SPI_MOSI_GPIO,
    ...
    ESP_RETURN_ON_ERROR(
        spi_bus_initialize(CONFIG_W5500_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO),
        TAG, "SPI host #%d init failed", CONFIG_W5500_SPI_HOST
    );
...
// ===== W5500 Backend Implementation =====

static esp_err_t w5500_backend_init(esp_eth_handle_t *out_handle) {
    ESP_LOGI(TAG, "Initializing W5500 SPI Ethernet controller");
    ESP_LOGI(TAG, "Configuration: CS=%d, INT=%d, RST=%d, PHY_ADDR=%d",
             CONFIG_W5500_SPI_CS_GPIO,
             CONFIG_W5500_INT_GPIO,
             CONFIG_W5500_PHY_RST_GPIO,
             CONFIG_W5500_PHY_ADDR);
...
const ethernet_backend_t ethernet_backend_w5500 = {
    .init = w5500_backend_init,
    .shutdown = w5500_backend_shutdown,
    .name = "W5500"
};
